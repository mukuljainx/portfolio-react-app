	...
	init: function(ajax) {
      ...
      $.ajax({
          ...
          for (var i = 0; i < events.length; i++) {
            $('.list').append('<div class="day-event" date-day="'+ events[i].day +'" date-month="' + events[i].month +'" date-year="'+ events[i].year +'" data-number="'+ i +'"><br><p>'+ events[i].title +'<br>Date : ' + events[i].day + '/' + events[i].month+ '/' + events[i].year + '<br>' + events[i].description +'</p><a href="##" class="close fontawesome-remove"></a></div>');
          }
    ...
    var mon = 'Mon';
		...
		function addEvent(){
			$('tbody.event-calendar td').on('click', function(e) {
				...
				var eventMonth = $(this).attr('date-month');
				var eventYear = $(this).attr('date-year');
				$('tbody.event-calendar td').removeClass('addeventselected');
				$(this).addClass('addeventselected');

				sessionStorage.setItem('day', eventDay);
				sessionStorage.setItem('month', eventMonth);
				sessionStorage.setItem('year', eventYear);
